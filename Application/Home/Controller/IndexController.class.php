...
//        print_r($this->queryStr);exit;
        ...
        if($_SESSION[C('ADMIN_AUTH_KEY')] || C('USER_AUTH_ON')==false){
            $sidecondition="and side='%d'";
            $sidecondition="";
            $f_menu = M('node as a')->where(" a.level='%d' AND status='%d' $sidecondition",array(1,1,$side))
                ->field("DISTINCT title,name,id,is_admin,model,sort,ico,default_open")
                ->order("sort ASC,id ASC")
                ->select();
            $s_menu = M('node as a')->where(" a.level='%d' AND status='%d' $sidecondition",array(2,1,$side))
                ->field("DISTINCT title,name,pid,id,is_admin,model,sort,ico,default_open")
                ->order("sort ASC,id ASC")
                ->select();
            $t_menu = M('node as a')->where(" a.level='%d' AND status='%d' $sidecondition",array(3,1,$side))
