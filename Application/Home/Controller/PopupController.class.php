...
class PopupController extends BasicController {
 ...
 // Category , @category , 分类
  ...
  private function Customer() {
    ...
    if($data ["search"] ["name"]){
          $where .= " and s.name like '%".$data ["search"] ["name"]."%'";
    }
    ...
    if($data["p"] > $tmp) {
      ...
      $data["popupdata"]=$model->join($join_condtion)
          ...
          ->where("$where_condition  $where")
          ->field("s.id,s.code,s.name,s.address,s.prefix,s.phone,s.linkman,s.status,pp.name as pname")
...
//    $data["popupdata"] = M()->query($sql);
    ...
    private function Subcode(){
        $data["funcid"] = I("get.funcid/s");
        $data["zindex"] = I("get.zindex/d");
        $data["p"] = I("get.p/d");
        $data["selecttype"] = I("get.selecttype");

        $data["treedatas"]=M('subcode ')->where('status=1')->field('id,name,code,parent_id')->order('parent_id ASC,sort ASC')->select();
    ...
    private function Department(){
        ...
        $data["treedatas"]=M('department')->where('parent_id = '.$data["parent_id"]." AND id <>".$data["excludeId"] ." and status=1")->field('id,name,code,parent_id')->order('sort')->select();
        ...
        $html = $this->fetch("Popup:DepartmentTree");
        echo $html;
    }


    // CustomerCategory , @customercategory , 客户分类
    private function Customerer() {
        $data["funcid"] = I("get.funcid/s");
        $data["zindex"] = I("get.zindex/d");
        $data["p"] = I("get.p/d");
        $data["selecttype"] = I("get.selecttype");

        $data["treedatas"]=M('customer')->where('status=1')->field('id,name,code,parent_id')->select();
        ...
        $html = $this->fetch("Popup:CustomererTree");
        echo $html;
    }
  ...
  private function CustomerCategory()
  {
      ...
      $data["excludeId"] = I("get.excludeId/d", 0);
      $onlyData = I("get.onlyData/d", 0);
      //$data["treedatas"] = M('customer_category')->where('status=1')->field('id,name,code,parent_id')->order('parent_id ASC,sort ASC')->select();
  ...
  */
      ...
      $data["treedatas"]=M('customer_category')->where('parent_id = '.$data["parent_id"]." AND id <>".$data["excludeId"] ." and status=1")->field('id,name,code,parent_id')->order('sort')->select();
  	...
  	$html = $this->fetch("Popup:CustomerCategoryTree");
  	echo $html;
  }
  ...
  // Department , @department , 部门禁用
  private function Departmenter() {
    $data["funcid"] = I("get.funcid/s");
    $data["zindex"] = I("get.zindex/d");
    $data["p"] = I("get.p/d");
    $data["selecttype"] = I("get.selecttype");
...
//   	}
  ...
  private function Product(){
    ...
    private function Goods(){
        ...
        $data ["page_size"] = I ( "get.pagesize/d" );
        $data ["page_size"] = $data ["page_size"] <= 0 ? session ( "Popup-Goods-PageSize" ) : $data ["page_size"];
        if (! $data ["page_size"]) {
            $data ["page_size"] = 8;
        }
        session ( "Popup-Goods-PageSize", $data ["page_size"] );
        ...
        $data["popupdata"] = $model->field("id, goods_no, name, brand, materials, style_info, producing_area, uom_weight")->where ( $where )->limit ( ($data ["p"] - 1) * $data ["page_size"], $data ["page_size"] )->select();
        $pageClass = new \Think\Page ( $count, $data ["page_size"] );
        $pageClass->rollPage = 3;
        $data ["page"] = $pageClass->show_drp ( $data ["funcid"], "编辑商品信息" );
  ...
  private function Goodsx(){
      ...
      $where['status'] = 1;
      $where['id'] = array('neq', $data['excludeId']);
      $where['parent_id'] = $data['parent_id'];
      $data["customers"]=M('goods_category')->where($where)->field('id,name,id as code,parent_id,code as xcode')->order('sort')->select();
      if($onlyData) {
          echo json_encode($data["customers"]);
          die;
      }
    ...
    private function GoodsCategory()
    {
        ...
        $data["treedatas"]=M('goods_category')->where('parent_id = '.$data["parent_id"]." AND id <>".$data["excludeId"] ." and status=1")->field('id,name,full_path,code,parent_id')->order('sort')->select();
        ...
        $html = $this->fetch("Popup:GoodsCategoryTree");
        echo $html;
    }
    ...
    /**
     * @author super
     * 新增的查询商品规格的方法用于GoodsAlias的 add
     */
    private function GoodsStyle()
    {
        $data["funcid"] = I("get.funcid/s");
        $data["zindex"] = I("get.zindex/d");
        $data["parent_id"] = I("get.parent_id/d", 0);
        ...
        $data["treedatas"]=M('goods_style')->where("status=1")->select();
        ...
        $html = $this->fetch("Popup:GoodsStyleTree");
        echo $html;
    }

    private function CustomerRela() {
        $data["funcid"] = I("get.funcid/s");
        $data["pfuncid"] = I("get.pfuncid/s");
        $data["zindex"] = I("get.zindex/d");
        $data["p"] = I("get.p/d");
        ...
        $data ["page_size"] = I ( "get.pagesize/d" );
        $data ["page_size"] = $data ["page_size"] <= 0 ? session ( "Popup-Customer-Rela-PageSize" ) : $data ["page_size"];
        if (! $data ["page_size"]) {
            $data ["page_size"] = 8;
        }
        session ( "Popup-Customer-Rela-PageSize", $data ["page_size"] );

        $where = "a.customer_id = ".$this->user["customer_id"];
        $model = M ( "customer_rela" );
        $count = $model->where($where)->count ();
        ...
        $data["list"] = $model->alias("a")->join("__CUSTOMER__ as b ON a.rela_id = b.id", "LEFT")->field("b.code, b.name, a.address, a.phone, a.linkman, a.mobile")->where ( $where )->limit ( ($data ["p"] - 1) * $data ["page_size"], $data ["page_size"] )->select();
        $pageClass = new \Think\Page ( $count, $data ["page_size"] );
        $pageClass->rollPage = 3;
        $data ["page"] = $pageClass->show_drp ( $data ["funcid"], "选择客户" );

        foreach ( $data as $key => $val ) {
            $this->assign ( $key, $val );
        }

        $html = $this->fetch("Popup:CustomerRela");
        echo $html;
    }

    private function CustomerDelivery() {
        $data["funcid"] = I("get.funcid/s");
        $data["pfuncid"] = I("get.pfuncid/s");
        $data["zindex"] = I("get.zindex/d");
        $data["p"] = I("get.p/d");
        ...
        $data ["page_size"] = I ( "get.pagesize/d" );
        $data ["page_size"] = $data ["page_size"] <= 0 ? session ( "Popup-Customer-Delivery-PageSize" ) : $data ["page_size"];
        if (! $data ["page_size"]) {
            $data ["page_size"] = 8;
        }
        session ( "Popup-Customer-Delivery-PageSize", $data ["page_size"] );

        $where = "customer_id = ".$this->user["customer_id"]." AND status = 1";
        if(!empty($carNo)) {
            $where .= " AND carno LIKE '%".$carNo."%'";
        }
        if(!empty($contact)) {
            $where .= " AND contact LIKE '%".$contact."%'";
        }
        ...
        $data["list"] = $model->field("id, carno, contact, idcard, phone, remarks")->where ( $where )->limit ( ($data ["p"] - 1) * $data ["page_size"], $data ["page_size"] )->select();
        $pageClass = new \Think\Page ( $count, $data ["page_size"] );
        $pageClass->rollPage = 3;
        $data ["page"] = $pageClass->show_drp ( $data ["funcid"], "选择提货信息" );

        foreach ( $data as $key => $val ) {
            $this->assign ( $key, $val );
        }

        $html = $this->fetch("Popup:CustomerDelivery");
        echo $html;
    }
