...
<link type="text/css" rel="stylesheet" href="/Public/css/Common/kuchain.css?t=<?php echo time(); ?>">
...
<script type="text/javascript" src="/Public/javascript/common.js?t=<?php echo time(); ?>"></script>
    <script type="text/javascript" src="/Public/javascript/project.js?t=<?php echo time(); ?>"></script>
...
<div class="main">
  <header class="header">
    <h1 class="sys-logo abe-fl"></h1>
     <?php if($user["side"]==1):?>
          <div class="sys-name"><em class="abe-space"></em><span>平台管理方</span></div>
     <?php endif; ?>
      <?php if($user["side"]==2):?>
      <div class="sys-name"><em class="abe-space"></em><span>联盟: <?php echo ($user['side_name']); ?></span></div>
      <?php endif; ?>
      <?php if($user["side"]==3):?>
      <div class="sys-name"><em class="abe-space"></em><span>客户: <?php echo ($user['side_name']); ?></span></div>
      <?php endif; ?>
        ...
        <?php if(is_array($s_menu)): foreach($s_menu as $key=>$vo2): if(($vo2['pid'] == $vo['id'])): if (!($vo2['is_admin'] == 1 && session(C("USER_AUTH_KEY"))!='admin')){ ?>
            ...
            <ul style="display:<?php echo $vo2['default_open']=='1'?'block':'none'; ?>" >
                <?php if(is_array($t_menu)): foreach($t_menu as $key=>$vo3): if(($vo3['pid'] == $vo2['id'])): if ($vo3['name'] == "-" || $vo3['title'] == "-"){ ?>
                        ...
                        <li><a href="<?php echo $vo3['module']?($vo3['model']=='open'?($vo3['module']):(U($vo3['module']))):''; ?>" data-type="<?php echo trim($vo3['model']) == ''?'link':$vo3['model'];?>" data-value="<?php echo ($vo3['name']); ?>"><i class="iconfont">&#xe60c;</i><em><?php echo ($vo3['title']); ?></em></a></li>
                        <?php } ?>
...
<!-- date:2019-6-14         <a href="javascript:void(0);" onclick="return _asr.openLink('<?php echo U("/Home/Notice/index?func=view");?>', 'notice_show', '<?php echo ($notice_title); ?>', 0)"><?php echo ($notice_title); ?>库链联盟</a>-->
        <a href="javascript:void(0);" onclick="return _asr.openLink('<?php echo U("/Home/Notice/index?func=view");?>', 'notice_show', '<?php echo ($notice_title); ?>', 0)">库链联盟</a>
    </div>
      <?php } ?>
    <div class="icp">
