...
<div class="prompt-pop pop-style2" id="<?php echo ($funcid); ?>" style="z-index:<?php echo ($zindex); ?>">
    ...
    <div class="title"><span class="pop-name">选择货品分类信息</span><a href="javascript:void(0);" class="close iconfont" onclick="_asr.closePopup('<?php echo ($funcid); ?>');">&#xe60d;</a> </div>
    <div class="pop-scroll">
        <div class="trees-nav trees-nav-pop abe-posclear" style="width: 100%; background:#fff;">
            ...
            <ul>
                ...
                <li><input j='<?php echo json_encode($v); ?>' data-id="<?php echo $v['code']; ?>" type="<?php echo (strtolower($selecttype)== 'multi'?'checkbox':'radio'); ?>" name="code[]" value="<?php echo $v['code']; ?>" show="<?php echo $v['name']; ?>"><?php echo $v['name']; ?></li>
                <?php } ?>
            </ul>
    ...
    <script>
        function <?php echo ($funcid); ?>_show(_t) {
            ...
            _this.attr("expand", "1");
            var url = "<?php echo U('/Home/Popup/index?func=GoodsCategory&excludeId='.$excludeId.'&selecttype='.$selecttype.'&onlyData=1');?>";
