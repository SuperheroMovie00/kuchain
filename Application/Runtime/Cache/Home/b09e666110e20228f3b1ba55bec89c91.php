...
<div toplayer="1" class="wrap-box wrap-pb2 " id="<?php echo $funcid;?>" summaryid="CustomerCategory" baseurl="<?php echo U('CustomerCategory/index'); ?>">

   <div class="wrap-box-info ">
      <div class="wrap-title abe-ofl">
         <div class="tit abe-fl">客户地址</div>
         <div class="abe-fr">
      ...
      <div class="table-box">
         <div class="table-in">
          ...
          <?php if ($search['status']==0): ?>
               <a href="javascript:void(0);"  class=" vi-blue vi-blue mlg_10 " onclick="return _asr.popupFun('<?php echo U("/Home/CustomerCategory/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/CustomerCategory/index?func=edit_base&id=$search[id]","");?>'); "> 编辑</a>
          <?php endif; ?>
          <?php endif; ?>
     ...
     <table border="0" cellspacing="0" cellpadding="0" class="pub-table-par">
        ...
        <tbody>
           ...
           <tr class= "even">
             <th>上级分类</th>
                 <td><?php echo get_table_CustomerCategory_byID("$search[parent_id]","code","") ; ?></td>
             <th>分类名称</th>
           ...
           <tr class= "odd">
             ...
             <th>描述</th>
             <td title="<?php echo $search["description"]; ?>"><?php echo OverView($search["description"],150,"..."); ?></td>
             <th>创建时间</th>
             <td><?php echo system_format("DT", $search["create_time"],1); ?><em class="abe-space-sm"></em><?php echo $search["create_user"]; ?></td>
             <th>状态</th>
             <td><?php echo get_table_CustomerCategory_status("$search[status]","name","") ; ?></td>
           </tr>
       ...
       <div class="table-box">
         <div class="table-in">
            <form action="<?php echo U('CustomerCategory/index?func=view'); ?>" id="<?php echo "$funcid"; ?>-Search" method="get">
                ...
                <input type="hidden" name="_tab_caozuorizhi_psize" value="<?php echo $search["_tab_caozuorizhi_psize"]; ?>" />
                  ...
                  <div class="order-det-ptab">
                    <div class="od-info abe-fl" >
                          <a href="#" onclick="return _asr.tabsheet('<?php echo $funcid;?>', '<?php echo "$funcid"; ?>-Search', 'kehuziliao');" <?php if($search['_tab'] == 'kehuziliao'): ?> class="current"<?php endif;?>>客户资料</a>
...
搜索关闭状态，按需要打开 -->
              ...
              <?php if ($search['_tab']=='kehuziliao'): ?>
                <div class="table-box">
                  <div class="table-in">
                    <form id="<?php echo "$funcid"; ?>-Result"  action="<?php U('CustomerCategory/op'); ?>" method="post">
                      <input type="hidden" name="<?php echo "$funcid"; ?>-last-url" id="<?php echo "$funcid"; ?>-last-url" value="<?php echo $__last_url; ?>" />
                      <input type="hidden" name="funcid" value="<?php echo "$funcid"; ?>" />
                      <input type="hidden" name="pfuncid" value="<?php echo "$pfuncid"; ?>" />
                      <input type="hidden" name="id" value="<?php echo $search["id"]; ?>" />
                      ...
                      <table border="0" cellspacing="0" cellpadding="0" class="pub-table-par pub-table-par-tdc pub-t-faq">
                        <colgroup>
                          ...
                          <col style="width: 80px ;"> <!-- 客户类型 -->
                          <col style="width: 80px ;"> <!-- 客户代码 -->
                          <col style="width: 100px ;"> <!-- 客户简称 -->
                          <col style="width: 100px ;"> <!-- 客户全称 -->
                          <col style="width: 80px ;"> <!-- 助记码 -->
                          <col style="width: 80px ;"> <!-- 客户地址 -->
                        ...
                        <tbody>
                          <tr>
                            ...
                            <th>状态</th>
                            <th>客户类型</th>
                            <th class="abe-txtl">客户代码</th>
                            <th class="abe-txtl">客户简称</th>
                            <th class="abe-txtl">客户全称</th>
                            <th>助记码</th>
                            <th class="abe-txtl">客户地址</th>
                          ...
                          <?php if(is_array($list)): $i = 0; $__LIST__ = $list; if( count($__LIST__) == 0) : echo ""; else: foreach($__LIST__ as $key=>$master): $mod = ($i % 2 );++$i; ?>
                                  <tr <?php if($mod == "1"): ?>class="even"<?php endif; if($mod == "0"): ?>class="odd"<?php endif; ?>>
                                    ...
                                    <td  class=" abe-txtl "><a href="javascript:void(0);" class="vi-blue"  onclick="return _asr.openLink('<?php echo U("/Home/Customer/index?func=view&id=$master[id]"); ?>','<?php echo filterFuncId( U("/Home/Customer/index?func=view&id=$master[id]") ,"");?>', '<?php echo tabTitle("客户","$master[code]") ; ?>' ,0); " ><?php echo $master["code"] ; ?></a></td>
                        ...
                        <tbody>
                          ...
                          <?php if(is_array($list)): $i = 0; $__LIST__ = $list; if( count($__LIST__) == 0) : echo ""; else: foreach($__LIST__ as $key=>$master): $mod = ($i % 2 );++$i; ?>
                                  <tr <?php if($mod == "1"): ?>class="even"<?php endif; if($mod == "0"): ?>class="odd"<?php endif; ?>>
                                    ...
                                    <td><?php echo get_table_CustomerCategory_status("$master[status]","name","") ; ?></td>
                                    <td><?php echo system_format("DT", $master["create_time"],1) ; ?></td>
   ...
   <div class="data-oper" >
      ...
      <div class="data-oper-in " >
          ...
          <?php if ($search['status']==0): ?>
               <input type="button" value="信息编辑" class="btn btn-blue mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/CustomerCategory/index?func=edit_base&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/CustomerCategory/index?func=edit_base&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']=='0'): ?>
               <input type="button" value="转有效" class="btn btn-blue mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/CustomerCategory/index?func=status_on&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/CustomerCategory/index?func=status_on&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']=='1'): ?>
               <input type="button" value="转无效" class="btn btn-org mlg_10 " default-status="1" onclick="return _asr.popupFun('<?php echo U("/Home/CustomerCategory/index?func=status_off&id=$search[id]") ; ?>', '<?php echo filterFuncId("/Home/CustomerCategory/index?func=status_off&id=$search[id]","");?>'); " />
          <?php endif; ?>
          <?php endif; ?>
          ...
          <?php if ($search['status']==0): ?>
               <input type="button" value="记录删除" class="btn btn-org mrg_10 " default-status="1" onclick="return _asr.confirm('确认操作', '请确认是否进行 信息删除 操作?', '', '<?php echo U("/Home/CustomerCategory/index?func=delete&id=$search[id]") ; ?>','','<?php echo "$funcid"; ?>-Result'); " />
          <?php endif; ?>
          <?php endif; ?>
   ...
   <div class="blank30"></div>
    ...
    <?php echo W('Summary/javascript',array('CustomerCategory'));?>
