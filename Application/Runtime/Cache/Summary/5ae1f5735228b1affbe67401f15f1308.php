...
<div toplayer="1" class="wrap-box wrap-nmd wrap-style00 " id="<?php echo $funcid;?>" summaryid="LogOrderSummary" baseurl="<?php echo U('LogOrderSummary/index'); ?>">
  <div class="wrap-box-info ">

    <div class="wrap-title abe-ofl">
      <div class="tit abe-fl" style="width: 450px;">凭据日志列表</div>
      <div class="abe-fl">
        <div class="textbox" >
          <div  id="<?php echo "$funcid"; ?>-keyword-info" class="abe-fl"<?php if($search["_showsearch"]=="show"):?>style="display:none;"<?php endif; ?>>
             ...
             <input type="button" value="搜索" class="btn btn-org  mrg_10 " id="<?php echo $funcid;?>-keyword-search"  default-status="1"  onclick="<?php echo ($funcid); ?>_keyword_set(); return _asr.submit('<?php echo "$funcid"; ?>', '<?php echo "$funcid"; ?>-Search', '<?php echo U("LogOrderSummary/index?func=search&") ; ?>',''); " />
             <input id="<?php echo $funcid;?>-search-buttc" type="button" value="导出" class="btn btn-blue mrg_10 " default-status="1" onclick="return _asr.showExport('<?php echo "$funcid"; ?>', '<?php echo "$funcid"; ?>-Search', '<?php echo U("LogOrderSummary/index?func=export&") ; ?>',<?php echo $p ?>); " />
          </div>
    ...
    <div class="screening"  >
      <form action="<?php echo U('LogOrderSummary/index?func=search'); ?>" id="<?php echo "$funcid"; ?>-Search" method="get">
                      ...
                      <div class="unit-mid">
                          <div class="dropdown" id="<?php echo ($funcid); ?>_type">
                             <select class="pbsele dropdown0" name="type">
                               ...
                               <?php
 $keyvalue = table_LogOrder_type(); if(is_array($keyvalue)){ $i = 0; $__LIST__ = $keyvalue; ?>
                  ...
                  <li>
                      <div class="unit-left"> 明细条数</div>
                      <div class="unit-mid">
                          <div class="textbox">
                              <input type="text" class="pbtxt txt1" name="details"  value="<?php echo $search['details']; ?>"  >
                              <em class="space-line">-</em>
                              <input type="text" class="pbtxt txt1" name="details2" value="<?php echo $search['details2']; ?>"  >
                          </div>
                          <div class="prompt" style="display:none">
                              <div class="error"><i class="iconfont">&#xe616;</i></div>
                          </div>
          ...
          <?php if (isset($rights['search']) && $rights['search']): ?>
               <input type="button" value="搜索" class="btn btn-org  mrg_10 " onclick="return _asr.submit('<?php echo "$funcid"; ?>', '<?php echo "$funcid"; ?>-Search', '<?php echo U("LogOrderSummary/index?func=search&") ; ?>',''); " />
          <?php endif; ?>
          ...
          <?php if (isset($rights['export']) && $rights['export']): ?>
               <input type="button" value="导出" class="btn btn-blue mrg_10 " onclick="return _asr.showExport('<?php echo "$funcid"; ?>', '<?php echo "$funcid"; ?>-Search', '<?php echo U("LogOrderSummary/index?func=export&") ; ?>',<?php echo $p ?>); " />
          <?php endif; ?>

                    <em class="pdr_20"></em>
                    <a href="javascript:void(0);" class="table-set <?php echo count($colshow)>0?"vi-blue":"abe-gray3"; ?>"  onclick="return _asr.popupFun('<?php echo U("/Summary/LogOrderSummary/index?func=columnsetting&") ; ?>','<?php echo filterFuncId("/LogOrderSummary/column","");?>');  " ><em class="iconfont abe-ft14 mrg_5">&#xe60a;</em>设置</a>
                    </div>
                  </div>
      </form>
    </div>
  ...
  <div class="wrap-nmd-box">
    <div class="wrap-master ">
      ...
      <div class="list-scroll">
        <div class="table-box">
          <div class="table-in">
            <form id="<?php echo "$funcid"; ?>-Result"  action="<?php U('LogOrderSummary/op'); ?>" method="post">
          ...
          <!-- 序号        <col style="width: 40px ;">   -->
                           ...
                           <col style="width: 100px ; <?php if(isset($colshow["subject"]) && $colshow["subject"]=="0"): ?>display:none;<?php endif; ?> ">  <!-- 标题 -->
                           <col style="width: 80px ; <?php if(isset($colshow["details"]) && $colshow["details"]=="0"): ?>display:none;<?php endif; ?> ">  <!-- 明细条数 -->
                           ...
                           <col style="width: 80px ; <?php if(isset($colshow["amount"]) && $colshow["amount"]=="0"): ?>display:none;<?php endif; ?> ">  <!-- 金额(元) -->
          ...
          <!-- 序号       <th>序号</th>   -->
                          ...
                          <th <?php if(isset($colshow["details"]) && $colshow["details"]=="0"): ?> style="display:none" <?php endif; ?>>明细条数</th>
                          ...
                          <th <?php if(isset($colshow["amount"]) && $colshow["amount"]=="0"): ?> style="display:none" <?php endif; ?> class="abe-txtr">金额(元)</th>
                           <th></th >
                        </tr>
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list; if( count($__LIST__) == 0) : echo ""; else: foreach($__LIST__ as $key=>$master): $mod = ($i % 2 );++$i; ?>
                                  ...
                                  <td style=" <?php if(isset($colshow["type"]) && $colshow["type"]=="0"): ?>display:none<?php endif; ?> " ><?php echo get_table_LogOrder_type("$master[type]","name","") ; ?></td>
                                  <td style=" <?php if(isset($colshow["order_no"]) && $colshow["order_no"]=="0"): ?>display:none<?php endif; ?> " class=" abe-txtl "><a href="javascript:void(0);" class="vi-blue"  onclick="return _asr.openLink('<?php echo orderurl("$master[type]"); ?>','<?php echo filterFuncId( orderurl("$master[type]") ,"");?>', '查看详情' ,0); " ><?php echo $master["order_no"] ; ?></a></td>
                                  <td style=" <?php if(isset($colshow["subject"]) && $colshow["subject"]=="0"): ?>display:none<?php endif; ?> " class=" abe-txtl "><?php echo $master["subject"] ; ?></td>
                                  <td style=" <?php if(isset($colshow["details"]) && $colshow["details"]=="0"): ?>display:none<?php endif; ?> " class=" <?php if ($master['details']<0): ?> abe-red <?php endif; ?> "><?php echo system_format("N3", $master["details"],1) ; ?></td>
    ...
    <script>
        ...
        function <?php echo $funcid; ?>_clearsearch(_frm){
            ...
            _asr.setvaluebyname(_frm,"details","");
            _asr.setvaluebyname(_frm,"details2","");
    ...
    <?php echo W('Summary/javascript',array('LogOrderSummary'));?>
