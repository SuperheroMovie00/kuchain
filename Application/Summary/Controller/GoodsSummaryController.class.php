...
//
//注释: GoodsSummary - 联盟货品列表
//
use Home\Controller\BasicController;
use Think\Log;
class GoodsSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'GoodsSummary', '/Home/Goods', '/Home/GoodsStyle', ));
        if ($funcs)
            ...
            $funcs = array(
                         ...
                         array("key"=>"master_edit","func"=>"/Home/GoodsStyle","Action"=>"create") ,
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/GoodsSummary/index?func=search&").  "','".filterFuncId("GoodsSummary_Search","id=0")."','联盟货品列表', 1",""));
    ...
    private function search($data) {
       ...
       $condition_goods = join_condition_auth($condition_goods,$auth_condition,"");
           //select fields
           ...
           if ($page_size<=0){
               $page_size = session("GoodsSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("GoodsSummary-PageSize", $page_size);
        ...
        $html = $this->fetch("GoodsSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        $condition_goods = $this->tabsheet_condition($condition_goods ,$search["_tab"]);
          $condition_goods = join_condition_shop($condition_goods,"2;@goods.org_id;org_id",$this->user_shop_id,$search_auth_codes);
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("GoodsSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("GoodsSummary", 'gbk', 'utf-8');
    ...
    private function get_auth_condition(){
        ...
        case "2":
            $condition.=" @goods.org_id='".$this->user["org_id"]."' ";
            break;
        }
        return $condition ;
    }
