...
//
//注释: LogChainSummary - 交易链日志列表
//
use Home\Controller\BasicController;
use Think\Log;
class LogChainSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'LogChainSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"LogChainSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"LogChainSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"LogChainSummary","Action"=>"export")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"LogChainSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "LogChainSummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/LogChainSummary/index?func=search&").  "','".filterFuncId("LogChainSummary_Search","id=0")."','交易链日志列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_log_chain = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_log_chain = join_condition2($condition_log_chain,"@log_chain.status",$search["status"],$search["status2"],"int");
               $condition_log_chain = join_condition2($condition_log_chain,"@log_chain.create_time",$search["create_time"],$search["create_time2"],"datetime");
               $condition_log_chain = join_condition($condition_log_chain,"@log_chain.chain_id",$search["chain_id"],"int");
               $condition_log_chain = join_condition($condition_log_chain,"@log_chain.chain_no",$search["chain_no"],"char");
               $condition_log_chain = join_condition($condition_log_chain,"@log_chain.subject",$search["subject"],"char","both");
               $condition_log_chain = join_condition2($condition_log_chain,"@log_chain.weight",$search["weight"],$search["weight2"],"decimal");
               $condition_log_chain = join_condition2($condition_log_chain,"@log_chain.qty",$search["qty"],$search["qty2"],"decimal");
               $condition_log_chain = join_condition2($condition_log_chain,"@log_chain.amount",$search["amount"],$search["amount2"],"decimal");
           }

           //增加 tab 条件
           $condition_log_chain = $this->tabsheet_condition($condition_log_chain ,$search["_tab"]);
           //select fields
           $selectfields=" @log_chain.status ";
           $selectfields.=", @log_chain.id ";
           $selectfields.=", @log_chain.create_time ";
           $selectfields.=", @log_chain.chain_id ";
           $selectfields.=", @log_chain.chain_no ";
           ...
           if ($page_size<=0){
               $page_size = session("LogChainSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("LogChainSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @log_chain  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_log_chain,$count_sql);
           ...
           $sql = "select #selectfields# from @log_chain  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_log_chain,$sql);
        ...
        $html = $this->fetch("LogChainSummary:index");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("LogChainSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @log_chain  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_log_chain,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        $str = mb_convert_encoding("LogChainSummary", 'gbk', 'utf-8');
