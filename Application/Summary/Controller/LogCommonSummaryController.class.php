...
//
//注释: LogCommonSummary - 公共日志列表
//
use Home\Controller\BasicController;
use Think\Log;
class LogCommonSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'LogCommonSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"LogCommonSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"LogCommonSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"LogCommonSummary","Action"=>"export")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"LogCommonSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "LogCommonSummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/LogCommonSummary/index?func=search&").  "','".filterFuncId("LogCommonSummary_Search","id=0")."','公共日志列表', 1",""));
    ...
    private function search($data) {
       ...
       $search["data_code"] = I("request.data_code");
       ...
       $condition="";
       $condition_log_common="";
       if($bsearch) {
           //关键字条件
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_log_common = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_log_common = join_condition2($condition_log_common,"@log_common.status",$search["status"],$search["status2"],"int");
               $condition_log_common = join_condition2($condition_log_common,"@log_common.create_time",$search["create_time"],$search["create_time2"],"datetime");
               $condition_log_common = join_condition($condition_log_common,"@log_common.type",$search["type"],"char");
               $condition_log_common = join_condition($condition_log_common,"@log_common.data_code",$search["data_code"],"char");
               $condition_log_common = join_condition($condition_log_common,"@log_common.subject",$search["subject"],"char","both");
           }

           //增加 tab 条件
           $condition_log_common = $this->tabsheet_condition($condition_log_common ,$search["_tab"]);
           //select fields
           ...
           if ($page_size<=0){
               $page_size = session("LogCommonSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("LogCommonSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @log_common  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_log_common,$count_sql);
           ...
           $sql = "select #selectfields# from @log_common  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_log_common,$sql);
        ...
        $html = $this->fetch("LogCommonSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        if ($show_list['data_code']==1 || empty($show_list)){
            $str_header .= "代码,";
        }
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("LogCommonSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @log_common  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_log_common,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        foreach($list as $master) {
            ...
            if ($show_list['type']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_LogCommon_type("$master[type]","name"))."\t,";
            }
            if ($show_list['data_code']==1  || empty($show_list )) {
                $str_line.=$this->csvdata($master["data_code"])."\t,";
            }
            if ($show_list['subject']==1  || empty($show_list )) {
        ...
        $str = mb_convert_encoding("LogCommonSummary", 'gbk', 'utf-8');
