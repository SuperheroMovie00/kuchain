...
use Think\Log;
class RoleSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/Role', 'RoleSummary', '/Home/RoleNode', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/Role","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"RoleSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"RoleSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"RoleSummary","Action"=>"export") ,
                         array("key"=>"master_view","func"=>"/Home/Role","Action"=>"view") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"RoleSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "RoleSummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/RoleSummary/index?func=search&").  "','".filterFuncId("RoleSummary_Search","id=0")."','角色列表', 1",""));
       ...
       if($bsearch) {
           ...
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   ...
                   $condition_role = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_role = join_condition($condition_role,"@role.status",$search["status"],"int");
               $condition_role = join_condition($condition_role,"@role.name",$search["name"],"char","both");
           }

           //增加 tab 条件
           $condition_role = $this->tabsheet_condition($condition_role ,$search["_tab"]);
           //select fields
           $selectfields=" @role.status ";
           $selectfields.=", @role.id ";
           $selectfields.=", @role.name ";
           $selectfields.=", '' newcolumn ";


           $page_size = I("request.pagesize/d");
           if ($page_size<=0){
               $page_size = session("RoleSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("RoleSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @role  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_role,$count_sql);
           ...
           $sql = "select #selectfields# from @role  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_role,$sql);
        ...
        $html = $this->fetch("RoleSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        if ($show_list['newcolumn']==1 || empty($show_list)){
            $str_header .= "权限设置,";
        }
        $str_header .= "\r\n";
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("RoleSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @role  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_role,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        foreach($list as $master) {
            $str_line="";
            if ($show_list['status']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_Role_status("$master[status]","name"))."\t,";
            }
            if ($show_list['name']==1  || empty($show_list )) {
        ...
        $str = mb_convert_encoding("RoleSummary", 'gbk', 'utf-8');
