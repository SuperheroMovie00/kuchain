...
class SubcodeSummaryController extends BasicController {
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/SubcodeSummary/index?func=search&").  "','".filterFuncId("SubcodeSummary_Search","id=0")."','数据字典列表', 1",""));
