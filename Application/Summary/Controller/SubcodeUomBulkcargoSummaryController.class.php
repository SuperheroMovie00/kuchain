...
//
//注释: SubcodeUomBulkcargoSummary - 数据字典 - 散件单位列表
//
use Home\Controller\BasicController;
use Think\Log;
class SubcodeUomBulkcargoSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/Subcode', 'SubcodeUomBulkcargoSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/Subcode","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"SubcodeUomBulkcargoSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"SubcodeUomBulkcargoSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"SubcodeUomBulkcargoSummary","Action"=>"export") ,
                         array("key"=>"master_view","func"=>"/Home/Subcode","Action"=>"view") ,
                         array("key"=>"master_edit","func"=>"/Home/Subcode","Action"=>"edit") ,
                         array("key"=>"master_delete","func"=>"/Home/Subcode","Action"=>"delete") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"SubcodeUomBulkcargoSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "SubcodeUomBulkcargoSummary";
        ...
        switch ($func)
        {
          ...
          //$this->ajaxResult("数据字典 - 散件单位列表后台错误");
    ...
    private function columnsetting($data){
        ...
        $data['summary']='SubcodeUomBulkcargoSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/SubcodeUomBulkcargoSummary/index?func=search&").  "','".filterFuncId("SubcodeUomBulkcargoSummary_Search","id=0")."','数据字典 - 散件单位列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("SubcodeUomBulkcargoSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("SubcodeUomBulkcargoSummary-PageSize", $page_size);
           ...
           $sql = "select #selectfields# from @subcode where type='UOM_BULKCARGO' #CONDITION# order by sort";
        ...
        $html = $this->fetch("SubcodeUomBulkcargoSummary:index");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("SubcodeUomBulkcargoSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @subcode where type='UOM_BULKCARGO' #CONDITION# order by sort";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_subcode,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        $str = mb_convert_encoding("SubcodeUomBulkcargoSummary", 'gbk', 'utf-8');
