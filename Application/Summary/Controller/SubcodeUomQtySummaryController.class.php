...
//
//注释: SubcodeUomQtySummary - 数据字典 - 数量单位列表
//
use Home\Controller\BasicController;
use Think\Log;
class SubcodeUomQtySummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/Subcode', 'SubcodeUomQtySummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/Subcode","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"SubcodeUomQtySummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"SubcodeUomQtySummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"SubcodeUomQtySummary","Action"=>"export") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"SubcodeUomQtySummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "SubcodeUomQtySummary";
        ...
        switch ($func)
        {
          ...
          //$this->ajaxResult("数据字典 - 数量单位列表后台错误");
    ...
    private function columnsetting($data){
        ...
        $data['summary']='SubcodeUomQtySummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/SubcodeUomQtySummary/index?func=search&").  "','".filterFuncId("SubcodeUomQtySummary_Search","id=0")."','数据字典 - 数量单位列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("SubcodeUomQtySummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("SubcodeUomQtySummary-PageSize", $page_size);
           ...
           $sql = "select #selectfields# from @subcode where type='UOM_QTY' #CONDITION# order by sort";
        ...
        $html = $this->fetch("SubcodeUomQtySummary:index");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("SubcodeUomQtySummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @subcode where type='UOM_QTY' #CONDITION# order by sort";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_subcode,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        $str = mb_convert_encoding("SubcodeUomQtySummary", 'gbk', 'utf-8');
