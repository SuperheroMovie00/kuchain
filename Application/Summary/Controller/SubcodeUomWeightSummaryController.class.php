...
//
//注释: SubcodeUomWeightSummary - 数据字典 - 重量单位列表
//
use Home\Controller\BasicController;
use Think\Log;
class SubcodeUomWeightSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/Subcode', 'SubcodeUomWeightSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/Subcode","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"SubcodeUomWeightSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"SubcodeUomWeightSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"SubcodeUomWeightSummary","Action"=>"export") ,
        ...
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"SubcodeUomWeightSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "SubcodeUomWeightSummary";
        ...
        switch ($func)
        {
          ...
          //$this->ajaxResult("数据字典 - 重量单位列表后台错误");
    ...
    private function columnsetting($data){
        ...
        $data['summary']='SubcodeUomWeightSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/SubcodeUomWeightSummary/index?func=search&").  "','".filterFuncId("SubcodeUomWeightSummary_Search","id=0")."','数据字典 - 重量单位列表', 1",""));
    ...
    private function search($data) {
       ...
       if($bsearch) {
           ...
           if ($page_size<=0){
               $page_size = session("SubcodeUomWeightSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("SubcodeUomWeightSummary-PageSize", $page_size);
           ...
           $sql = "select #selectfields# from @subcode where type='UOM_WEIGHT' #CONDITION# order by sort";
        ...
        $html = $this->fetch("SubcodeUomWeightSummary:index");
        echo $html;
    }
    ...
    private function export(){
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("SubcodeUomWeightSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @subcode where type='UOM_WEIGHT' #CONDITION# order by sort";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_subcode,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        $str = mb_convert_encoding("SubcodeUomWeightSummary", 'gbk', 'utf-8');
