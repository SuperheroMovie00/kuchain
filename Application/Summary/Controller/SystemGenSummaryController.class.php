...
//
//注释: SystemGenSummary - 系统序号列表
//
use Home\Controller\BasicController;
use Think\Log;
class SystemGenSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'SystemGenSummary', '/Home/%table%', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"SystemGenSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"SystemGenSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"SystemGenSummary","Action"=>"export") ,
                         array("key"=>"status","func"=>"/Home/%table%","Action"=>"%action%")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"SystemGenSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "SystemGenSummary";
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/SystemGenSummary/index?func=search&").  "','".filterFuncId("SystemGenSummary_Search","id=0")."','系统序号列表', 1",""));
    ...
    private function search($data) {
       ...
       $search["mainkey"] = I("request.mainkey");
       $search["subkey"] = I("request.subkey");
       $search["seqno"] = I("request.seqno");
       $search["seqno2"] = I("request.seqno2");
       ...
       $condition="";
       $condition_system_gen="";
       if($bsearch) {
           //关键字条件
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_system_gen = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_system_gen = join_condition($condition_system_gen,"@system_gen.type",$search["type"],"char");
               $condition_system_gen = join_condition($condition_system_gen,"@system_gen.mainkey",$search["mainkey"],"char");
               $condition_system_gen = join_condition($condition_system_gen,"@system_gen.subkey",$search["subkey"],"char");
               $condition_system_gen = join_condition2($condition_system_gen,"@system_gen.seqno",$search["seqno"],$search["seqno2"],"int");
               $condition_system_gen = join_condition2($condition_system_gen,"@system_gen.create_time",$search["create_time"],$search["create_time2"],"datetime");
           }

           //增加 tab 条件
           $condition_system_gen = $this->tabsheet_condition($condition_system_gen ,$search["_tab"]);
           //select fields
           $selectfields=" @system_gen.id ";
           $selectfields.=", @system_gen.type ";
           $selectfields.=", @system_gen.mainkey ";
           $selectfields.=", @system_gen.subkey ";
           $selectfields.=", @system_gen.seqno ";
           $selectfields.=", @system_gen.create_time ";


           $page_size = I("request.pagesize/d");
           if ($page_size<=0){
               $page_size = session("SystemGenSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("SystemGenSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @system_gen  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_system_gen,$count_sql);
           ...
           $sql = "select #selectfields# from @system_gen  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_system_gen,$sql);
        ...
        $html = $this->fetch("SystemGenSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        if ($show_list['mainkey']==1 || empty($show_list)){
            $str_header .= "主键,";
        }
        if ($show_list['subkey']==1 || empty($show_list)){
            $str_header .= "子键,";
        }
        if ($show_list['seqno']==1 || empty($show_list)){
            $str_header .= "最后序号,";
        }
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("SystemGenSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @system_gen  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_system_gen,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        foreach($list as $master) {
            ...
            if ($show_list['mainkey']==1  || empty($show_list )) {
                $str_line.=$this->csvdata($master["mainkey"])."\t,";
            }
            if ($show_list['subkey']==1  || empty($show_list )) {
                $str_line.=$this->csvdata($master["subkey"])."\t,";
            }
            if ($show_list['seqno']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(system_format("N3", $master["seqno"]))."\t,";
            }
            if ($show_list['create_time']==1  || empty($show_list )) {
        ...
        $str = mb_convert_encoding("SystemGenSummary", 'gbk', 'utf-8');
