...
//
//注释: TradeAssignSummary - 交易配货列表
//
use Home\Controller\BasicController;
use Think\Log;
class TradeAssignSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( '/Home/TradeAssign', 'TradeAssignSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"add","func"=>"/Home/TradeAssign","Action"=>"add") ,
                         array("key"=>"refresh","func"=>"TradeAssignSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"TradeAssignSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"TradeAssignSummary","Action"=>"export") ,
                         array("key"=>"master_view","func"=>"/Home/TradeAssign","Action"=>"view") ,
                         array("key"=>"master_edit","func"=>"/Home/TradeAssign","Action"=>"edit") ,
                         array("key"=>"master_delete","func"=>"/Home/TradeAssign","Action"=>"delete")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"TradeAssignSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "TradeAssignSummary";
    ...
    private function columnsetting_define(){
        return array(
            ...
            'act_bulkcargo'=>'实发散件',
            'act_time'=>'扣除时间',
            'buyer_storecard_id'=>'买家存储卡',
            'buyer_storecard_no'=>'买家存储卡',
            'create_time'=>'创建时间',
        );
    }
    ...
    private function columnsetting($data){
        ...
        $data['summary']='TradeAssignSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/TradeAssignSummary/index?func=search&").  "','".filterFuncId("TradeAssignSummary_Search","id=0")."','交易配货列表', 1",""));
    ...
    private function search($data) {
       ...
       $condition="";
       $condition_trade_assign="";
       if($bsearch) {
           //关键字条件
           if($search["_showsearch"]=="hide"  ){
               if($search["_keyword"]){
                   $condition_keyword = "";
                   $condition_keyword = join_condition($condition_keyword,"@trade_assign.trade_no",$search["_keyword"],"char", "both" , 0, "" );
                   $condition_trade_assign = " AND ( ". $condition_keyword .")";
               }
           }else{
               //高级搜索condition
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.status",$search["status"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.org_id",$search["org_id"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.customer_id",$search["customer_id"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.customer_name",$search["customer_name"],"char","both");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.trade_id",$search["trade_id"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.trade_no",$search["trade_no"],"char");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.warehouse_code",$search["warehouse_code"],"char");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.storecard_id",$search["storecard_id"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.storecard_no",$search["storecard_no"],"char");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.storecard_type",$search["storecard_type"],"int");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.weight",$search["weight"],$search["weight2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.qty",$search["qty"],$search["qty2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.bulkcargo",$search["bulkcargo"],$search["bulkcargo2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.lock_time",$search["lock_time"],$search["lock_time2"],"datetime");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.release_time",$search["release_time"],$search["release_time2"],"datetime");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.act_weight",$search["act_weight"],$search["act_weight2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.act_qty",$search["act_qty"],$search["act_qty2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.act_bulkcargo",$search["act_bulkcargo"],$search["act_bulkcargo2"],"decimal");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.act_time",$search["act_time"],$search["act_time2"],"datetime");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.buyer_storecard_id",$search["buyer_storecard_id"],"int");
               $condition_trade_assign = join_condition($condition_trade_assign,"@trade_assign.buyer_storecard_no",$search["buyer_storecard_no"],"char");
               $condition_trade_assign = join_condition2($condition_trade_assign,"@trade_assign.create_time",$search["create_time"],$search["create_time2"],"datetime");
           }

           //增加 tab 条件
           $condition_trade_assign = $this->tabsheet_condition($condition_trade_assign ,$search["_tab"]);
           //增加数据权限
       $auth_condition=$this->get_auth_condition();
       $condition_trade_assign = join_condition_auth($condition_trade_assign,$auth_condition,"");
           //select fields
           ...
           $selectfields.=", @trade_assign.buyer_storecard_no ";
           $selectfields.=", @trade_assign.create_time ";


           $page_size = I("request.pagesize/d");
           if ($page_size<=0){
               $page_size = session("TradeAssignSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("TradeAssignSummary-PageSize", $page_size);


           $join="";
           $count_sql = "select count(*) as cnt from @trade_assign  #join#  where 1=1 #condition#";  // ' for skip replace $condition
           $count_sql = str_replace("#join#",$join,$count_sql);
           $count_sql = str_replace("#condition#",$condition_trade_assign,$count_sql);
           ...
           $sql = "select #selectfields# from @trade_assign  #join# Where 1=1 #condition# #orderby#";
           $sql = str_replace("#selectfields#",$selectfields,$sql);
           $sql = str_replace("#join#",$join,$sql);
           $sql = str_replace("#condition#",$condition_trade_assign,$sql);
        ...
        $html = $this->fetch("TradeAssignSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        $condition_trade_assign = $this->tabsheet_condition($condition_trade_assign ,$search["_tab"]);
          $condition_trade_assign = join_condition_shop($condition_trade_assign,"2;@trade_assign.org_id;org_id#3;@trade_assign.customer_id;customer_id",$this->user_shop_id,$search_auth_codes);
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("TradeAssignSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $sql = "select #selectfields# from @trade_assign  #join# Where 1=1 #condition# #orderby#";   // ' for skip replace $selectfields,$condition,$orderby
        $sql = str_replace("#selectfields#",$selectfields,$sql);
        $sql = str_replace("#join#",$join,$sql);
        $sql = str_replace("#condition#",$condition_trade_assign,$sql);
        $sql = str_replace("#orderby#",$orderby,$sql);
        ...
        foreach($list as $master) {
            $str_line="";
            if ($show_list['status']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_TradeAssign_status("$master[status]","name"))."\t,";
            }
            if ($show_list['org_id']==1  || empty($show_list )) {
            ...
            if ($show_list['storecard_type']==1  || empty($show_list )) {
                $str_line.=$this->csvdata(get_table_TradeAssign_storecard_type("$master[storecard_type]","name"))."\t,";
            }
            if ($show_list['weight']==1  || empty($show_list )) {
        ...
        $str = mb_convert_encoding("TradeAssignSummary", 'gbk', 'utf-8');
    ...
    private function tabsheet_condition($scondition, $itab)
    {
        ...
        switch($itab)
        {
            case 'all':  //全部
                 ...
                 break;
            case 'fou':  //否
                 $scond="@trade_assign.status='0'";
                 break;
            case 'kucunsuoding':  //库存锁定
                 $scond="@trade_assign.status='1'";
                 break;
            case 'shifangkouchu':  //释放扣除
                 $scond="@trade_assign.status='2'";
                 break;
            default :
                 $scond="";
                 break;
    ...
    private function get_auth_condition(){
        ...
        case "2":
            $condition.=" @trade_assign.org_id='".$this->user["org_id"]."' ";
            break;
        case "3":
            $condition.=" @trade_assign.customer_id='".$this->user["customer_id"]."' ";
            break;
        }
        return $condition ;
    }
