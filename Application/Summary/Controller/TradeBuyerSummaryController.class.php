...
//
//注释: TradeBuyerSummary - 交易开单列表
//
use Home\Controller\BasicController;
use Think\Log;
class TradeBuyerSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'TradeBuyerSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"TradeBuyerSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"TradeBuyerSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"TradeBuyerSummary","Action"=>"export")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"TradeBuyerSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "TradeBuyerSummary";
    ...
    private function columnsetting($data){
        ...
        $data['summary']='TradeBuyerSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/TradeBuyerSummary/index?func=search&").  "','".filterFuncId("TradeBuyerSummary_Search","id=0")."','交易开单列表', 1",""));
    ...
    private function search($data) {
       ...
       $condition_trade = join_condition_auth($condition_trade,$auth_condition,"");
           ...
           if ($page_size<=0){
               $page_size = session("TradeBuyerSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("TradeBuyerSummary-PageSize", $page_size);
        ...
        $html = $this->fetch("TradeBuyerSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        $condition_trade = $this->tabsheet_condition($condition_trade ,$search["_tab"]);
          $condition_trade = join_condition_shop($condition_trade,"3;@trade.buyer_id;customer_id#2;@trade.org_id;org_id",$this->user_shop_id,$search_auth_codes);
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("TradeBuyerSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("TradeBuyerSummary", 'gbk', 'utf-8');
    ...
    private function get_auth_condition(){
        ...
        case "3":
            $condition.=" @trade.buyer_id='".$this->user["customer_id"]."' ";
            break;
        case "2":
