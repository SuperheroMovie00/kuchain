...
//
//注释: TradeCustomerSummary - 交易开单列表
//
use Home\Controller\BasicController;
use Think\Log;
class TradeCustomerSummaryController extends BasicController {

    public function _init() {
        $funcs = $this->getUserRoleList($this->user,array( 'TradeCustomerSummary', ));
        if ($funcs)
            $this->assign("rights",  $funcs);
        else{
            $funcs = array(
                         array("key"=>"refresh","func"=>"TradeCustomerSummary","Action"=>"refresh") ,
                         array("key"=>"search","func"=>"TradeCustomerSummary","Action"=>"search") ,
                         array("key"=>"export","func"=>"TradeCustomerSummary","Action"=>"export")
             );
            $this->assign("rights",  $this->GetUserRights($this->user["id"],$funcs ));
        }
        $this->assign("colshow", $this->GetUserColumnDefine($this->user["id"],"TradeCustomerSummary"));
    }
    ...
    public function index() {
        ...
        if(empty($data["funcid"])) $data["funcid"] = "TradeCustomerSummary";
    ...
    private function columnsetting($data){
        ...
        $data['summary']='TradeCustomerSummary';
    ...
    private function columnsetting_save($data){
        ...
        $this->ajaxResult("", "",  array("_asr.closeTab","_asr.closePopup", "_asr.openLink","_asr.hideMask()"), array("$('#".$data["funcid"]."-Tab').find('a'), '".$data["funcid"]."'","'".$data["funcid"]."'", "'". U("/Summary/TradeCustomerSummary/index?func=search&").  "','".filterFuncId("TradeCustomerSummary_Search","id=0")."','交易开单列表', 1",""));
    ...
    private function search($data) {
       ...
       $condition_trade = join_condition_auth($condition_trade,$auth_condition,"");
           ...
           if ($page_size<=0){
               $page_size = session("TradeCustomerSummary-PageSize");
               if (!$page_size) {
                    $page_size = 10;
               }
           }
           session("TradeCustomerSummary-PageSize", $page_size);
        ...
        $html = $this->fetch("TradeCustomerSummary:index");
        echo $html;
    }
    ...
    private function export(){
        ...
        $condition_trade = $this->tabsheet_condition($condition_trade ,$search["_tab"]);
          $condition_trade = join_condition_shop($condition_trade,"3;@trade.customer_id;customer_id#2;@trade.org_id;org_id",$this->user_shop_id,$search_auth_codes);
       ...
       $count = $count[0]["cnt"];
        ...
        if(!$export_all) {
           $page_size = I("request.pagesize/d");
           $page_size = $page_size <= 0 ? session("TradeCustomerSummary-PageSize") : $page_size;
           if(!$page_size) {
              $page_size = 20;
           }
    ...
    for ($p;$p<=$total_page;$p++)
    {
        ...
        $str = mb_convert_encoding("TradeCustomerSummary", 'gbk', 'utf-8');
